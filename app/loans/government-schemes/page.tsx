...
export default function GovernmentSchemesPage() {
    ...
    return (
        <div className="pb-20 bg-white text-white font-sans mx-auto min-h-screen">
                ...
                <section className="relative pt-12 md:pt-20 pb-20 overflow-hidden bg-white text-white border-b border-slate-200">
                    ...
                    <div className="container px-4 md:px-6 mx-auto text-center max-w-4xl space-y-6 relative z-10">
                        <div className="inline-flex items-center gap-2 rounded-full border border-sky-500/30 bg-sky-50 px-4 py-1.5 text-xs font-black text-[#0284c7] uppercase tracking-widest">
                            <Landmark className="h-4 w-4 text-[#0284c7]" />
                        ...
                        <h1 className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tight leading-tight text-slate-900">
                            ...
                            <span className="text-[#0284c7]">Made Simple.</span>
                        </h1>
            ...
            <section className="py-16 container px-4 mx-auto max-w-5xl">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                    {featuredSchemes.map((scheme, index) => (
                        <Card key={index} className="bg-white border border-slate-200 rounded-3xl p-6 text-white shadow-sm hover:shadow-md hover:-translate-y-1 transition-all">
                            <CardHeader className="p-0 pb-4 flex flex-row items-center justify-between">
                                <div className="p-3 bg-[#0284c7]/15 border border-[#0284c7]/30 rounded-2xl text-[#0284c7]">
                                    <scheme.icon className="h-7 w-7" />
                                </div>
                                <span className="text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full bg-[#0284c7]/20 text-[#0284c7] border border-[#0284c7]/40">
                                    {scheme.badge}
                                </span>
                            </CardHeader>
                            <CardContent className="p-0 space-y-4">
                                <CardTitle className="text-2xl font-black text-slate-900">{scheme.title}</CardTitle>
                                <p className="text-slate-600 text-xs leading-relaxed font-medium">{scheme.description}</p>
                                <Link href={scheme.link} className="inline-flex items-center gap-2 text-xs font-black text-[#0284c7] hover:underline">
            ...
            <section className="py-12 container px-4 mx-auto max-w-5xl">
                <div className="grid md:grid-cols-3 gap-6">
                    {otherSchemes.map((item, i) => (
                        <div key={i} className="p-6 bg-white rounded-3xl border border-slate-200 text-white space-y-3">
                            <div className="p-2.5 bg-[#0284c7]/15 border border-[#0284c7]/30 rounded-xl text-[#0284c7] w-fit">
                    ...
                    <CardContent className="p-8 space-y-4">
                        {isSuccess ? (
                            <div className="text-center py-8 space-y-3">
                                ...
                                <p className="text-xs text-slate-500">Our government scheme advisor will contact you shortly.</p>
                            </div>
                        ) : (
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div>
                                    <label className="text-[10px] font-black uppercase text-slate-500">Preferred Govt Scheme</label>
                                    <Input
